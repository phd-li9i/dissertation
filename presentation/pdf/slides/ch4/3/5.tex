\begin{frame}{\texttt{pf} $+$ \texttt{sm2}}


  \noindent\makebox[\linewidth][c]{%
  \begin{minipage}{\linewidth}
    \begin{minipage}{0.3\linewidth}
      \begin{figure}
        \includegraphics[width=120pt]{./figures/slides/ch4/smsm_pf.eps}
        \caption{\tiny  \texttt{sm2} προσαρμοσμένη στην έξοδο φίλτρου σωματιδίων (\texttt{pf})}
      \end{figure}
    \end{minipage}
    \hspace{1.2cm}
    \begin{minipage}{0.6\linewidth}
      \begin{figure}\vspace{-0.09cm}
        \input{./figures/slides/ch4/sm_smsm_2.tikz}
        \caption{\tiny  \texttt{sm2}: ευθυγράμμιση πραγματικής σάρωσης με εικονική σάρωση χάρτη}
      \end{figure}
    \end{minipage}

  \end{minipage}
  }

\note{\footnotesize
Με αυτόν τον τρόπο μπορούμε να προσαρμόσουμε τη μέθοδο sm2 στην έξοδο του
φίλτρου σωματιδίων, όπου λόγω θορύβου μέτρησης και ατελούς αναπαράστασης του
χάρτη σε σχέση με το περιβάλλον, η τελική έξοδος της μεθόδου sm2 φέρει και αυτή
σφάλμα, και συνεπώς αποτελεί μία \textbf{δεύτερη} εκτίμηση.}

\end{frame}
