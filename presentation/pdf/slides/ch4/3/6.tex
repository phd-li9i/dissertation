\begin{frame}{\texttt{pf} $+$ \texttt{sm2} $+$ feedback}


  \noindent\makebox[\linewidth][c]{%
  \begin{minipage}{\linewidth}
    \begin{minipage}{0.3\linewidth}
      \begin{figure}
        \includegraphics[width=120pt]{./figures/slides/ch4/smsm_pf_feedback.eps}
        \caption{\tiny \texttt{sm2} προσαρμοσμένη στην έξοδο φίλτρου σωματιδίων (\texttt{pf}) και ανάδραση της εξόδου της}
      \end{figure}
    \end{minipage}
    \hspace{1.2cm}
    \begin{minipage}{0.6\linewidth}
      \begin{figure}
        \input{./figures/slides/ch4/sm_smsm_2.tikz}
        \caption{\tiny  \texttt{sm2}: ευθυγράμμιση πραγματικής σάρωσης με εικονική σάρωση χάρτη}
      \end{figure}
    \end{minipage}

  \end{minipage}
  }

\note{\footnotesize
Αυτή η δεύτερη εκτίμηση είναι μία υπόθεση για την οποία το ίδιο το φίλτρο δεν
έχει γνώση, και συνεπώς θα ήταν ωφέλιμο, εάν η υπόθεση φέρει όντως μικρότερο
σφάλμα, να εισαχθεί στον πληθυσμό του, ώστε το σφάλμα του ίδιου του φίλτρου
να μειωθεί.}


\end{frame}
