...
if exist('/media/li9i/var2/elements/PhD/fourier_scan_to_map_scan_matcher/struct/scripts_phd/experiments/improvement_percent/d_data/final_minus_initial_error') == 2 && ...
   exist('/media/li9i/var2/elements/PhD/fourier_scan_to_map_scan_matcher/struct/scripts_phd/experiments/improvement_percent/d_data/initial_orientation_errors_comb') == 2
  mats_exist = true;
end
...
if (~mats_exist)
  ...
  % Grab pose, orientation, and orientation errors per dataset
  initial_poses = {};
  real_poses = {};
  orientation_errors = {};

  for i = 1:size(datasets,2)
    orientation_errors{end+1} = function_get_orientation_errors(logs_dir, datasets{i});
  ...
  orientation_errors_comb = {};
  initial_poses_comb = {};
  real_poses_comb = {};

  % Do the first dataset just for later concatenation purposes
  for a = 1:size(orientation_errors{1},2)              % num algorithms
    for sm = 1:size(orientation_errors{1}{a},2)        % num map noises
      for sr = 1:size(orientation_errors{1}{a}{sm},2)  % num scan noises
        orientation_errors_comb{a}{sm}{sr} = orientation_errors{1}{a}{sm}{sr};
  ...
  for d = 2:size(orientation_errors,2)                   % num datasets
    for a = 1:size(orientation_errors{d},2)              % num algorithms
      for sm = 1:size(orientation_errors{d}{a},2)        % num map noises
        for sr = 1:size(orientation_errors{d}{a}{sm},2)  % num scan noises
          orientation_errors_comb{a}{sm}{sr} = [orientation_errors_comb{a}{sm}{sr}, orientation_errors{d}{a}{sm}{sr}];
  ...
  initial_orientation_errors_comb = {};
  for a = 1:size(orientation_errors{1},2)              % num algorithms
    for sm = 1:size(orientation_errors{1}{a},2)        % num map noises
      for sr = 1:size(orientation_errors{1}{a}{sm},2)  % num scan noises
        ...
        % Wrap angles
        d = rem(d+5*pi,2*pi)-pi;
        orientation_errors_comb{a}{sm}{sr} = rem(orientation_errors_comb{a}{sm}{sr}+5*pi,2*pi)-pi;

        initial_orientation_errors_comb{a}{sm}{sr} = abs(d);
        final_minus_initial_error{a}{sm}{sr} = abs(orientation_errors_comb{a}{sm}{sr}) - initial_orientation_errors_comb{a}{sm}{sr};
      end
    end
  end

  save final_minus_initial_error final_minus_initial_error
  save initial_orientation_errors_comb initial_orientation_errors_comb
else
  load('/media/li9i/var2/elements/PhD/fourier_scan_to_map_scan_matcher/struct/scripts_phd/experiments/improvement_percent/d_data/final_minus_initial_error');
  load('/media/li9i/var2/elements/PhD/fourier_scan_to_map_scan_matcher/struct/scripts_phd/experiments/improvement_percent/d_data/initial_orientation_errors_comb');
end
...
%\definecolor{c9}{RGB}{255, 0, 255}
      ...
      %\put(3999,5900){\makebox(0,0){\strut{}\footnotesize ως προς προσανατολισμό ανά μονάδα αρχικού σφάλματος εκτίμησης προσανατολισμού $|\Delta\hat{\theta}|$}}%
      %\put(0,5400){\makebox(0,0){\strut{}{\color{c1}{\rule[0.6mm]{0.5cm}{0.5mm}}}\scriptsize PLICP}}
      %\put(1000,5400){\makebox(0,0){\strut{}{\color{c2}{\rule[0.6mm]{0.5cm}{0.5mm}}}\scriptsize NDT}}
      %\put(2500,5400){\makebox(0,0){\strut{}{\color{c3}{\rule[0.6mm]{0.5cm}{0.5mm}}}\scriptsize GICP}}
      %\put(3000,5400){\makebox(0,0){\strut{}{\color{c4}{\rule[0.6mm]{0.5cm}{0.5mm}}}\scriptsize VGICP}}
      %\put(6300,5400){\makebox(0,0){\strut{}{\color{c7}{\rule[0.6mm]{0.5cm}{0.5mm}}}\scriptsize \texttt{x1}}}
...
set(gca(), "xtick", [100*pi/16 100*pi/8 100*pi/4], "xticklabel", {'','',''});
set(gca(), "ytick", [85 90 95 100], "yticklabel", {'\\small $85\\%$', '\\small $90\\%$', '\\small $95\\%$', '\\small $100\\%$'});
xlim(xlim0)
ylim(ylim0)
...
ylim(ylim0)
set(gca(), "xtick", [100*pi/16 100*pi/8 100*pi/4], "xticklabel", {'','',''});
set(gca(), "ytick", [85 90 95 100], "yticklabel", {'','','',''});
...
xlim(xlim5)
ylim(ylim5)
set(gca(), "xtick", [100*pi/16 100*pi/8 100*pi/4], "xticklabel", {'\\small $\\frac{pi}{16}$','\\small $\\frac{pi}{8}$','\\small $\\frac{pi}{4}$'});
set(gca(), "ytick", [85 90 95 100], "yticklabel", {'\\small $85\\%$', '\\small $90\\%$', '\\small $95\\%$', '\\small $100\\%$'});
...
if store
  img_file = strcat(print_dir, '/orientation_improvement_percent_binned_curves.eps');
